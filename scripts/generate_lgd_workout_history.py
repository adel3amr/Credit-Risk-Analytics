...
def main():
    ...
    buckets = np.array([6,12,24,36,60])
    ...
    # Workout costs are treated as negative recovery cash flows at 12 months.
    net_cfs = recovery_cfs.copy()
    net_cfs[:,1] = np.maximum(net_cfs[:,1] - workout_cost, -workout_cost)
    ...
    df = pd.DataFrame({
        ...
        "recovery_cf_6m": net_cfs[:,0].round(2),
        "recovery_cf_12m": net_cfs[:,1].round(2),
        "recovery_cf_24m": net_cfs[:,2].round(2),
        "recovery_cf_36m": net_cfs[:,3].round(2),
        "recovery_cf_60m": net_cfs[:,4].round(2),
